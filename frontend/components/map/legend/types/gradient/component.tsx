...
export const LegendTypeGradient: FC<LegendTypeGradientProps> = ({ className = '', items }: LegendTypeGradientProps) => (
  <div
    className={cx({
      [className]: !!className,
    })}
  >
    ...
    <ul className="flex justify-between w-full mt-1">
      {items
        .filter(({ value }) => !!value)
        .map(({ value }) => (
          <li key={`${value}`} className="flex-shrink-0 text-xs text-blue-800">
            {value}
          </li>
        ))}
    </ul>
